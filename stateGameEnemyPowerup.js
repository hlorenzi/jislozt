function StateGameEnemyPowerup()
{
	this.kind = 0;
...
StateGameEnemyPowerup.prototype.AnimatePieceBulletHit = function()
{
	this.pieceHitAnimation = 1;
}

StateGameEnemyPowerup.prototype.PiecePlayfield_BreakPiece = function(blockX, blockY, color)
{
	this.AddFallingBlock(blockX, blockY, color);
}

StateGameEnemyPowerup.prototype.PiecePlayfield_ClearedLines = function(lineNum)
{
	if (this.life <= 0)
		return;
	...
	if (this.life <= 0)
	{
		...
		{
			state.AddScore(3500);
			state.specialScore = new StateGameSpecialScore(this.x + this.pieceField.pieceWidth * BLOCK_SIZE / 2, this.y, 6);
			SoundStop("mscGame");
			SoundPlay("mscPowerup", 1);
			state.player.powerupTimer = state.player.powerupTimerMax;
		}
		else // No Bonus
		{
...
StateGameEnemyPowerup.prototype.Destroy = function()
{
	this.life = 0;
	this.deathAnimation = 0;
	for (var i = 0; i < 100; i++)
...
StateGameEnemyPowerup.prototype.MakeFallRemainingBlocks = function()
{
...
StateGameEnemyPowerup.prototype.AddFallingBlock = function(blockX, blockY, color)
{
...
StateGameEnemyPowerup.prototype.Update = function()
{
...
StateGameEnemyPowerup.prototype.Draw = function()
{
	ctx.save();
	ctx.globalAlpha = (1 - this.deathAnimation / 30);
	
	var bodyColor = "#FFCC88";
	...
	for (var i = 0; i < 5; i++)
	{
		var angleBig = i / 5.0 * Math.PI * 2;
		var angleSmall = (i + 0.5) / 5.0 * Math.PI * 2;
	
		ctx.lineTo(Math.cos(angleBig) * 7, Math.sin(angleBig) * 7);
		ctx.lineTo(Math.cos(angleSmall) * 4, Math.sin(angleSmall) * 4);
	}
	ctx.fill();
	ctx.restore();
